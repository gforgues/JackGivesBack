 ...
 public class GameEngine implements Observable {
   ...
   */
   ...
   //for (int i=0;i<getNumberOfPlayers();i++){
    System.out.println(player.getUsername() + " how much do you want to bet?");
    ...
    try { 
     ...
     while (a > playersAndChips.get(player).getChips()) {
      //add back chips after evaluating while loop condition
      //playersAndChips.get(player).addChips(player, a);

      System.out.println("sorry not enough chips, please enter " + playersAndChips.get(player).getChips()  + " or less" );
...
//       System.out.println(!(b.get(j).isBust()) +"isbust");
       ...
       System.out.println("now for your current hand, do you want to hit, stand, double down, split?");
    ...
    * Compute all winners
    */
   for (int i1=0;i1<getNumberOfPlayers();i1++){
    
    ArrayList<BlackjackHand> b1 = playersAndHands.get(players.get(i1));
    
    if (b1.size() == 1) {
     if (processWinner(b1.get(0)) == 1) 
      System.out.println("Dealer wins!");
     if (processWinner(b1.get(0)) == -1) {
      if (b1.get(0).checkBlackjack()) {
//       playersAndChips.get(gameTable.getPlayer(i)).addChips(players.get(i), BLACKJACK_PAYOUT_CONSTANT * playersAndChips.get(gameTable.getPlayer(i)).getBet());
       players.get(i1).addWin(BLACKJACK_PAYOUT_CONSTANT * playersAndChips.get(players.get(i1)).getBet());
       System.out.println("BLACKJACK! X2 PAYOUT! " + gameTable.getPlayer(i1).getUsername()+ " wins!");
      } else {
       players.get(i1).addWin(WIN_CONSTANT * playersAndChips.get(players.get(i1)).getBet());
//       playersAndChips.get(gameTable.getPlayer(i)).addChips(players.get(i), WIN_CONSTANT * playersAndChips.get(gameTable.getPlayer(i)).getBet());
       System.out.println(gameTable.getPlayer(i1).getUsername()+ " wins!");
      }
     }
     if (processWinner(b1.get(0)) == 0) {
      playersAndChips.get(gameTable.getPlayer(i1)).addChips(players.get(i1), playersAndChips.get(gameTable.getPlayer(i1)).getBet());
